...
inline void Model<Array_Type,Data_Counter_Type,Data_Rule_Type, Data_Rule_Dyn_Type>::update_pset_probs(
    ...
    size_t n_params = params.size();
    pset_probs.resize(
        pset_locations.back() + // Where the last pset starts.
        pset_sizes.back()       // How many elements the last pset has.
        );
    ...
    for (size_t s = 0u; s < pset_sizes.size(); ++s)
    {
        ...
        // If seen before, then no need to update the probs
        if (vec_equal_approx(params, params_last_pset[s]))
            continue;
        ...
        for (size_t a = 0u; a < pset_sizes[s]; ++a)
        {
            ...
            // Start location in the array is defined by the location
            // of the pset_prob (how many arrays before it), plus
            // the location in that sequence (a), times the number of
            // parameters in the model. 
            size_t start_loc = (pset_start + a) * n_params;
...
inline Model<Array_Type,Data_Counter_Type,Data_Rule_Type, Data_Rule_Dyn_Type>::Model(
    ...
    params_last_pset(Model_.params_last_pset),
...
Model<Array_Type,Data_Counter_Type, Data_Rule_Type, Data_Rule_Dyn_Type>::add_array(
    ...
    if (force_new | (locator == keys2support.end()))
    {
        ...
        if (with_pset)
        {
            ...
            pset_sizes.push_back(pset_arrays.back().size());
        ...
        params_last_pset.push_back(stats_target[0u]);
...
Model<Array_Type,Data_Counter_Type,Data_Rule_Type, Data_Rule_Dyn_Type>::sample(
    ...
    // Sampling an array
    size_t j = 0u;
    if (vec_equal_approx(params, params_last[a]) && vec_equal_approx(params, params_last_pset[a]))
    ...
    if (locator == keys2support.end())
    {
        ...
        if (with_pset)
        {
            ...
            catch (const std::exception& e)
            {
                ...
                err = err + std::string("with error ") + e.what() + std::string("\n");
